...
m_tableWidget[ska_num][STATUS0] ->setRowCount(m_inputs_map[ska_num][STATUS0].size());
...
void SKA_DD_RIOM::set_data_struct(const QByteArray &input, const SKA_VEHICLE_NUM &ska_x_num)
{
    ...
    if (input.size() == sizeof(m_ska_x_dd_riom[ska_num])) {
        ...
        memcpy(&m_ska_x_dd_riom[ska_num].ska_dd_riom_ccu_mvb1_inputs.bytes, input.constData(), sizeof(m_ska_x_dd_riom[ska_num].ska_dd_riom_ccu_mvb1_inputs.bytes));
